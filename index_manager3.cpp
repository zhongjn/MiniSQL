...
void IndexManager::remove_item(string& scheme, Type type, int degree, Value value, int num)
{
	BlockGuard bg(block_mgr, scheme, num);
	treenode* tree_info = bg.addr<treenode>();
	int i;
	...
	if (tree_info->Is_leaf == false)
	{
		...
		int block_index = get_block(scheme, type, num, degree, i);
		remove_item(scheme, type, degree, value, i);
	...
	else
	{
		...
		else
		{
			...
			{
				neighbor_bg.set_modified();
				merge_leaf(scheme, type, neighbor_num, num, degree);
			}
		}
	}
}
void IndexManager::delete_item(const Relation& rel, int field_index, const Value& value, int num)
{
	string& scheme = Files::index(rel.name, field_index);
	Type type = rel.fields[field_index].type;
	tree_information tree_info = get_information(rel, field_index, scheme);
	...
	remove_item(scheme, type, degree, value, tree_info.root);
